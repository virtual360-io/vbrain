...
- REPO = `__VBRAIN_REPO__`
- Prefixo pra rodar qualquer script:
  `BUNDLE_GEMFILE=__VBRAIN_REPO__/Gemfile bundle exec ruby __VBRAIN_REPO__/scripts/<script>.rb`
...
```
tags.rb --limit 80
query.rb "<termos que você acha que deveriam casar>" --no-log --format json
```
...
```
ingest_raw.rb <tmpfile.md> --type text     # devolve {"raw_id": N, ...}
```
...
`wiki/` nem usa `rm`/`mv` na mão; o `write_pages.rb` encena tudo numa temp e só
então aplica de uma vez (mesmo processo do add-knowledge):

```
write_pages.rb --raw-id <N> --pages-json <pages.json>
```
...
(merge) ou o hub citar esses raws — e rode o `write_pages.rb` de novo. Só siga
...
```
commit.rb --message "dream: reorg a partir de N queries (hubs/tags/merges)"
```

Se a base não tiver repo git, `commit.rb` é no-op — apenas siga.

### 7. Esvazie a fila que você processou

```
query_log.rb --prune --through-id <max_id do passo 1>
```
