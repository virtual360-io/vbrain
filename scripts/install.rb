...
# Instala também na base (~/vbrain), tornando-a autossuficiente: CLAUDE.md +
# skills cruas (paths relativos, diferente do target global que reescreve pra
# absoluto) + cópia do código (scripts/, lib/, Gemfile...) que as skills usam.
base = VBrain::Paths.data_home
puts
if opts[:dry_run]
  skill_count = Dir.children(SKILLS_SRC).count { |c| File.directory?(File.join(SKILLS_SRC, c)) }
  puts "base (#{base}): (dry-run) escreveria CLAUDE.md + #{skill_count} skills cruas + código (scripts/, lib/, Gemfile)"
else
  summary = VBrain::Scaffold.install!(base)
  committed = false
  if VBrain::Git.repo_initialized?(base) && VBrain::Git.changes?(base)
    VBrain::Git.commit!("chore: assets do agente vbrain na base (CLAUDE.md + skills + código)", base)
    committed = true
  end
  puts "base (#{base}): CLAUDE.md=#{summary['claude_md']} skills=#{summary['skills_installed']} " \
       "código=#{summary['code_installed']} committed=#{committed}"
end
